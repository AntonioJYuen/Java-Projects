...
public class JiraReader{
		...
		public void retrieveInfo() throws URISyntaxException, InterruptedException, ExecutionException, MalformedURLException{
...
//	        System.out.println(intro.toString());
	        ...
	        try {
	        	...
	        	JOptionPane.showMessageDialog(null, "Wrong username/password combination", "Wrong username/password combination", JOptionPane.ERROR_MESSAGE);
	        	return;
	        }
	        ...
	        try {
		        ...
		        for(Issue issue : searchJqlPromise.claim().getIssues()) {
		        	...
		        	for(IssueField i : issue.getFields()){
		        		...
		        		//Return Description
		        		if (issue.getIssueType().getName().equals(global_Form_Enhancement) || issue.getIssueType().getName().equals(global_Form_Regulatory_Change)){
		        		...
		        		if (i.getName().trim().equals(affected_Components)){
		        			...
		        			for(int j=0;j<temp.length;j++){
		        				...
		        				case "FreeFormReport": 	csImpact = csImpact + "F";
		        										break;
		        				case "Edit Check": 		csImpact = csImpact + "E";
		        										break;
		        				case "Cosmetic": 		csImpact = csImpact + "C";
		        										break;
		        				default: 				csImpact = csImpact + "O";
		        										break;
		        				}
		        			}
...
//		      		System.out.println("Item #: " + issueNum + " | Key: " + jKey + " | Description: " + csDesc.replaceAll("\r\n", " ") + " | Impact: " + csImpact + 
//		      				" | Change_Type: " + csType + " | Version: " + vID + " | Change Details: " + cdDetails.replaceAll("\r\n", " ") + " | Change Tab:" + cdTab);
		        		        	
		      		this.project.addTicket(new Ticket(vID, form, issueNum, jKey, csDesc, csImpact, csType, cdDetails, cdTabStr, 0, cv_Branch, qa_Actual_Result));
	        	...
	        	JOptionPane.showMessageDialog(null, "Please check your Ticket Filters", "Please check your Ticket Filters", JOptionPane.ERROR_MESSAGE);
	        	return;	        	
	        }
	    }
